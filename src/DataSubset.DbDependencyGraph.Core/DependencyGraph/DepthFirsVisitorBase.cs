...
namespace DataSubset.DbDependencyGraph.Core.DependencyGraph
{
    ...
    /// <list type="bullet">
    ///   <item><description><see cref="GetSkipCurrentNode(TableNode, ITableDependencyEdgeData?, int)"/> to decide whether a node and its subtree are skipped.</description></item>
    ///   <item><description><see cref="GetNodeContext(TableNode, ITableDependencyEdgeData?, int)"/> to create a per-node context that is passed to child visits.</description></item>
    ///   <item><description><see cref="ProcessCurrentNode(TableNode, ITableDependencyEdgeData?, int, TNodeContext?)"/> to perform work for each visited node.</description></item>
    /// </list>
    /// <para>
    ...
    public abstract class DepthFirsVisitorBase<TNodeContext>(DatabaseGraph dbGraph)
    {
        /// <summary>
        ...
        /// <see cref="GetSkipCurrentNode(TableNode, ITableDependencyEdgeData?, int)"/>; when it returns <see langword="true"/>,
        ...
        public void VisitTablePostOrder((string schema, string tableName)[] rootTables)
        {
            foreach (var table in rootTables)
            {
                ...
                VisitTablePostOrder(rootEdge, depth: 0, default);
            }
        }
        ...
        /// <param name="parentNodeContext">The context computed for the parent node, if any; passed to <see cref="ProcessCurrentNode(TableNode, ITableDependencyEdgeData?, int, TNodeContext?)"/>.</param>
        /// <remarks>
        /// Invokes <see cref="GetSkipCurrentNode(TableNode, ITableDependencyEdgeData?, int)"/> before descending; if skipped, the subtree is not visited.
        /// Calls <see cref="GetNodeContext(TableNode, ITableDependencyEdgeData?, int)"/> to compute the current node's context that will be supplied to children.
        /// After visiting all children, calls <see cref="ProcessCurrentNode(TableNode, ITableDependencyEdgeData?, int, TNodeContext?)"/>.
        /// </remarks>
        private void VisitTablePostOrder(GraphEdge<TableNode, ITableDependencyEdgeData> dependencyEdge, int depth, TNodeContext? parentNodeContext)
        {
        ...
        /// and to <see cref="ProcessCurrentNode(TableNode, ITableDependencyEdgeData?, int, TNodeContext?)"/> for the corresponding node,
        /// depending on traversal order.
        /// </remarks>
        protected abstract TNodeContext GetNodeContext(TableNode currentTable, ITableDependencyEdgeData? data, int depth);
        ...
        /// <param name="parentNodeContext">The context computed for the parent node, if any; passed to <see cref="ProcessCurrentNode(TableNode, ITableDependencyEdgeData?, int, TNodeContext?)"/>.</param>
        /// <remarks>
        /// Invokes <see cref="GetSkipCurrentNode(TableNode, ITableDependencyEdgeData?, int)"/> before processing; if skipped, the subtree is not visited.
        /// Processes the current node first via <see cref="ProcessCurrentNode(TableNode, ITableDependencyEdgeData?, int, TNodeContext?)"/>, then descends into children.
        /// Calls <see cref="GetNodeContext(TableNode, ITableDependencyEdgeData?, int)"/> to compute the context to supply to child visits.
        /// </remarks>
        private void VisitTablPreOrder(GraphEdge<TableNode, ITableDependencyEdgeData> dependencyEdge, int depth, TNodeContext? parentNodeContext)
        {
            var currentTable = dependencyEdge.Target;
            var dependencies = dbGraph.GetOutgoingEdges(currentTable);
        ...
        /// </returns>
        protected abstract bool GetSkipCurrentNode(TableNode currentTable, ITableDependencyEdgeData? data, int depth);
        ...
        /// The context associated with the current node, as produced by <see cref="GetNodeContext(TableNode, ITableDependencyEdgeData?, int)"/>.
        ...
        /// </remarks>
        protected abstract void ProcessCurrentNode(TableNode currentTable, ITableDependencyEdgeData? data, int depth, TNodeContext? currentNodeContext);
    }
}
